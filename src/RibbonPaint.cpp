...
{	
	...
	_splashScreen = ci::gl::Texture( loadImage( loadResource( SPLASH_SCREEN ) ) );
...
void RibbonPaint::mouseDown( ci::app::MouseEvent event )
{
...
void RibbonPaint::mouseMove( ci::app::MouseEvent event )
{
	if(_state != kStateNormal) return;
...
void RibbonPaint::mouseUp( ci::app::MouseEvent event )
{	
	if(_state == kStateIsShowingSplashScreen)
	{
		if( getElapsedFrames() < 210 ) return;
		setupAfterSplash();
		return;
	} 	
...
{	
	...
	if (_state < kStateNormal)
	{
		...
		float blackLevel = ci::math<float>::clamp(((float) getElapsedFrames()) / 120, 0, 1.0f);
		blackLevel = -1.0 * cos(blackLevel/1.0 * (M_PI/2)) + 1.0 + 0.0;
		
		glColor4f( blackLevel, blackLevel, blackLevel, blackLevel );
		
		glEnable( GL_TEXTURE_2D );
		ci::gl::draw( _splashScreen, ci::Rectf(0, 0, getWindowWidth(), getWindowHeight() ) );
		...
		if(_latestVersion > _versionNumber)
		{
			...
			float alpha = fabs( sinf( ((getElapsedFrames()*0.006) * M_PI * 2) ) );
			if(blackLevel > 0.75f)
...
//		gl::drawSolidRect(Rectf(0, 0, getWindowWidth(), getWindowHeight() ) );
	...
	if(_alertTextDisplayTimer > 0)
	{
		...
		if(doDrawString)
		{
			...
			ci::ColorA stringColor = (_colorMode < COLORMODE_GRAYSCALE) ? ci::ColorA(1.0, 1.0, 1.0, (float) _alertTextDisplayTimer / (float) _alertTextDefaultDisplayTimeInFrames) : ci::ColorA(0.0, 0.0, 0.0, (float) _alertTextDisplayTimer / (float) _alertTextDefaultDisplayTimeInFrames);
			ci::gl::drawStringCentered(_alertTextString, ci::Vec2f(getWindowCenter().x, 20.0f), stringColor, ci::Font("Arial", 10) );
		}
