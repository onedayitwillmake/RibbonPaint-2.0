...
{	
	...
	{
		if( _getElapsedFrames() < 210 ) return;
		setupAfterSplash();
		return;
	} 	
...
{	
	...
	if (_state < kStateNormal)
	{
		...
		float blackLevel = ci::math<float>::clamp(((float) _getElapsedFrames()) / 120, 0, 1.0f);
		blackLevel = -1.0 * cos(blackLevel/1.0 * (M_PI/2)) + 1.0 + 0.0;

		glColor4f( blackLevel, blackLevel, blackLevel, blackLevel );

		glEnable( GL_TEXTURE_2D );
		ci::gl::draw( _splashScreen, ci::Rectf(0, 0,  _getWindowWidth(),  _getWindowHeight() ) );
		...
		if(_latestVersion > _versionNumber)
		{
			...
			float alpha = fabs( sinf( ((_getElapsedFrames()*0.006) * M_PI * 2) ) );
			if(blackLevel > 0.75f)
	...
	if(_drawParams) {
		...
		if(instructionsOverlay)
		{
			...
			glEnable( GL_TEXTURE_2D );
			ci::gl::draw( instructionsOverlay, ci::Vec2f(10,  _getWindowHeight() - instructionsOverlay.getHeight() - 25 ) );
			glDisable( GL_TEXTURE_2D );	
			...
			ci::gl::drawSolidRect(ci::Rectf(0, _getWindowHeight()-25 , instructionsOverlay.getWidth(),  _getWindowHeight() ) );
			//glColor4f( 1.0, 1.0, 1.0, 1.0 );
		}
	}
	...
	if(_alertTextDisplayTimer > 0)
	{
		...
		if(doDrawString)
		{
			...
			ci::ColorA stringColor = (_colorMode < COLORMODE_GRAYSCALE) ? ci::ColorA(1.0, 1.0, 1.0, (float) _alertTextDisplayTimer / (float) _alertTextDefaultDisplayTimeInFrames) : ci::ColorA(0.0, 0.0, 0.0, (float) _alertTextDisplayTimer / (float) _alertTextDefaultDisplayTimeInFrames);
			ci::gl::drawStringCentered(_alertTextString, ci::Vec2f(_getWindowCenter().x, 20.0f), stringColor, ci::Font("Arial", 10) );
		}
